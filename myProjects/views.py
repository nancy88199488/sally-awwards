...
from .forms import UpdateUserProfileForm,UpdateUserForm,SignUpForm,MyProjectsForm,RatingForm
...
from .serializer import ProfileSerializer, myProjectSerializer

# Create your views here.
def index(request):
    myProjects = myProjectSerializer.objects.all()
...
def searchprofile(request):
    if 'searchUser' in request.GET and request.GET['searchUser']:
        ...
        searchResults = MyProjects.search_projects(name)
        message = f'name'
        params = {
...
def addProject(request):
    ...
    if request.method == 'POST':
        form = MyProjectsForm(request.POST,request.FILES)
        if form.is_valid:
...
class ProjectList(APIView):
    def get(self,request,format = None):
        all_projects = MyProjects.objects.all()
        serializerdata = myProjectSerializer(all_projects,many = True)
        return Response(serializerdata.data)
